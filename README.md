...
`DefaultAgentLoop` 已打通 Client → Conversation pending 基础路径，支持非流式完整响应、
流式 `StreamEvent` 透传、provider-neutral tool-use 执行编排和 result 回灌，并在无 tool-use 的
final assistant 后提交 Turn、发出 `StepBoundary` 与 `Done`；pivot 已改为 `AgentInput::Pivot`：
由 driver / Session 在合法的 tool-result step boundary 直接把 `Role::User` 消息注入同一
pending turn（sans-io 机器的 `step(External(AgentInput::Pivot))`），沿用现有 role sequence
校验，注入后在同一 step 下重新渲染 LLM 请求推进本 turn，不再走库内 pivot queue；
...
client、tool registry、MCP session、approval responder 和 task handle 留在单独 runtime
holder；`RunContext` 本身也不作为 serde data shape 持久化；`AgentFeedGuard` 保证同一
Agent 的前一段 feed stream 消费完或 drop 前不能再次 feed。
已完成的 Client 层实施记录见
...
当前 Agent 层阶段计划和任务见 [`PLAN.md`](PLAN.md) 与 [`TODO.md`](TODO.md)。
