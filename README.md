...
data-only `LoopCursor`，以及 run 级 `RunContext` 取消、预算和 trace handle 边界；
...
holder；`RunContext` 本身也不作为 serde data shape 持久化。
已完成的 Client 层实施记录见
...
`AgentState`/`LoopCursor` 可恢复状态边界和 `RunContext` 横切上下文边界，具体 Agent loop、
