...
consolidate，同时保留 raw history 和已替换 artifact provenance。
已完成的 Client 层实施记录见
...
- `conversation`：外部注入的强类型 id、独立 system 配置、不可原地修改的消息 envelope、
  ...
  或 compaction apply 而改写。

Conversation Core 正按任务顺序继续实现 compaction strategy/trigger 扩展点与持久化；
Agent loop、Tool registry 与多 agent 编排仍不在范围内。完整设计和当前阶段
