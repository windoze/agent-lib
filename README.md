...
revert/redo，移动只裁剪当前有效视图并重建派生 index，不删除 raw Turn；从回退位置再次
提交会生成新的 parent suffix，旧分支仍可只读调试。
已完成的 Client 层实施记录见
...
- `conversation`：外部注入的强类型 id、独立 system 配置、不可原地修改的消息 envelope、
  ...
  `raw_turns` 分别公开有效前缀、可 redo lineage 和 retained raw 分支的只读视图。

Conversation Core 正按任务顺序继续实现 fork、projection 与
