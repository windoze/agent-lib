...
final assistant 后提交 Turn、发出 `StepBoundary` 与 `Done`；
...
- `agent`：外部注入的强类型 Agent/Run/Step/ToolSet/Skill/Plan/Blackboard id，以及字段私有、
  ...
  `AgentState` 持有唯一活动 `Conversation`、active skills、queued pivot/reconfig 数据和
  ...
  与 tool-use LLM 往返，调用方需在 `AgentInput::UserMessage` 中同时注入初始 user 与 assistant
  message id。
...
非流式/流式 LLM step、tool-use 执行编排、tool result 回灌与 Conversation pending commit 集成；
