...
### NEXT-1 [DONE] 归档 Client 层计划并建立 Conversation Core 计划
**前置条件与上下文**:
...
**完成记录**:
- 2026-07-13:在覆盖根文件前将 Client 层 `PLAN.md`/`TODO.md` 原样复制到固定归档目录;完成状态写入前以 Git blob hash 复核,两份归档分别与任务开始时 `HEAD:PLAN.md`/`HEAD:TODO.md` 完全一致。随后仅在本归档任务条目更新 `[DONE]` 与完成记录,保留 M1--M6 全部历史、验证结果和 NEXT-1 正文。
- 以 `docs/conversation-core.md` 全文和现有 Client 公共类型为依据重建根计划,明确 immutable message、I1--I4 commit 门、pending/cancel、受检 Boundary、logical head、O(1) fork、Projection/compaction 与一致点持久化;同时将 `ToolResult` 无法无损保存 Cancelled 状态的现有缺口排为 pending 前置修复,未使用 `extra` workaround。
- 根 `TODO.md` 建立 M1--M6 六个依赖有序里程碑和 28 个未执行任务;结构化审查确认每项均有前置依赖、上下文、实施内容与验证,实现任务编号在各里程碑内连续唯一,每个里程碑以独立 `Mx-R [TODO]` Review 收尾,无预先 `[DONE]` 标题。
- 更新 README 与 capability matrix 的计划引用:当前实施入口继续指向根 `PLAN.md`/`TODO.md`,Client 完成记录和 endpoint 约定改指归档。验证通过:`cargo fmt --all -- --check`,`cargo clippy --all-targets -- -D warnings`,`cargo test --all --all-targets`(133 passed,7 ignored),`RUSTDOCFLAGS="-D warnings" cargo doc --no-deps`,`git diff --check`;全部本地测试在 1 分钟内完成。
