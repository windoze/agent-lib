...
### [DONE] H-1 归档当前计划并为 facade API 生成新的 PLAN/TODO
...
- 归档:把切换前的完整 `PLAN.md`(Managed External Agent 计划)与本 `TODO.md`(M1–M10 + H-1,
  本任务标 `[DONE]` 后的最终态)完整复制到
  [`docs/archive/2026-07-17-managed-external-agent/`](../docs/archive/2026-07-17-managed-external-agent/)
  (沿用既有 `docs/archive/YYYY-MM-DD-<topic>/{PLAN,TODO}.md` 惯例;主题名唯一,避免与同日
  `2026-07-17-effect-refine`、`2026-07-17-external-agent` 冲突)。归档副本内容完整、可读。
- 新根 `PLAN.md`:以 [`docs/facade-api.md`](docs/facade-api.md) 为唯一设计输入重写,含目标/非目标、
  现有代码锚点(client/conversation/agent 装配面)、6 个 milestone(Chat facade → 基础 Agent facade →
  Local subagent → Managed external agent → Dispatcher/Escalator → Collaboration,承接 facade-api.md
  §18 落地顺序)、把 facade-api.md §20 七个未定问题列为「待确认风险/文档缺口」(含 schemars 当前非核心
  依赖这一实证缺口),以及默认完整验证序列。
- 新根 `TODO.md`:重写为 facade 落地任务单。开头写通用执行规则(一次只做首个 `[TODO]`、完成改 `[DONE]`
  并补完成记录);任务按实现顺序编号 `M<里程碑>-<序号>`,全部未完成任务标题带 `[TODO]`;每个任务含
  「上下文/做什么/验证条件」三段;每个 milestone 末尾有独立 `M<n>-R` review 任务。
- 验证:归档文件存在且完整;新 PLAN 明确引用 `docs/facade-api.md` 且含 milestone/风险/验证;新 TODO
  编号+`[TODO]`+每 milestone review+三段结构齐全;`git diff --check` 通过。本任务为纯文档(*.md)改动,
  不改任何编译产物,复用上一轮(M10-3)绿测结果,未重跑 `cargo test --all --all-targets`。
