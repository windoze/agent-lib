...
- **worktree isolation**:示例与 e2e 每个 child 都在 OS temp 下自建一次性 `git init` worktree,
  结束即删,写文件绝不触碰启动它的 checkout（详见 [`managed-external-agent.md`](./managed-external-agent.md) §16）。
