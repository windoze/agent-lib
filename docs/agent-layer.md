...
> 2. **pivot / skill 激活等一切"运行时 context 变更",归一到"在 `Boundary` 处向
>    pending 注入"这一套机制**(见 §4)。
...
4. **需要 conversation 层新开一个"边界注入入口"。**
   ...
   序列合法(tool_result 后接 user 合法)与 tool 配对。
...
1. **conversation 需要一个"边界注入入口"**:在 step 边界向 pending turn 追加
   user/system 消息(pivot / skill 激活共用),注入后仍满足 role 合法 + tool 配对
   (§4)。当前 `PendingTurn`(一个 turn 绑一条 user 消息)尚无此入口。
