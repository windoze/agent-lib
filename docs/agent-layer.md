...
facade `AgentBuilder::budget(BudgetLimits)` / `AgentRestoreBuilder::budget(BudgetLimits)`
负责把宿主配置的预算放进每次根 `RunContext`:顶层 run 之间 ledger 重新开始,同一次 run 内的
supervisor、子 agent 与 managed external delegate 共享同一个 budget handle。facade 将
`Done(BudgetExhausted)` 映射为 `FacadeError::BudgetExhausted`,而不是成功输出或普通字符串错误。
...
| **cancel** | 不是单独的取消流,而是对在途 `Requirement` 喂 `StepInput::Abandon(id)`,即 **never-resume** | driver 在三个观测点检查 `RunContext.cancel`:批次 fulfil 前、批次等待**期间**(M3-3:cancel 命中时先给合作 handler 一个有界 unwind 宽限跑清理尾,宽限到期仍阻塞的 fulfill future 被 drop/detach,不再等它)、fulfil 返回后(resume 前)(M4-5);命中即对批内**全部**未兑现 requirement 逐一 `NeverResumed` 留痕并 abandon,机器经 `Conversation::cancel_pending` 闭合 pending;闭合后仍可再喂(硬性验收标准);`TurnDone.cancelled()` 区分取消与自然结束 |
...
M2-2 后,facade 的非流式与流式 drive scope 都提供 `ReconfigRegistryHandler`。facade 构建时给
machine 装配同一个 facade 专用 `ToolRegistryResolver`;queue-time validation 与 apply-time swap
共享该实例,目标 `ToolSetRef` 的名字集合必须来自已注册的 facade tool surface。每个 run 的初始
...
重试即可(H-STATE-5 / M4-2)。若 driver 对该 registry requirement 选择 **never-resume**
(abandon),during-turn park 折叠好的文本响应不再被丢弃:pending 停在 `ReadyToCommit`
(`ResumeTurn` 在该相位非法),机器改为 `commit_pending` 提交文本、把被放弃的
reconfiguration 留在队列,下个 turn 边界重发(M4-4)。
...
- 调度器读取同一个 `RunContext` 预算:预算接近阈值时可以降级到 cheaper worker;若
  `budget_exhausted()` 已返回某个维度,必须硬停止并返回 `BudgetExhausted`,不得继续派 cheapest
  worker 消耗已经没有 headroom 的 run。
