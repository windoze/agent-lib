...
已知限制:reconfigure 准入可接受的工具名集合在 build/restore 时按当时注入的声明**冻结**;
若注入的 custom registry 在运行期动态增删声明,准入面不会随之更新(新声明不可经
`ReplaceToolSet` 激活,被删声明的名字仍可准入但执行由 registry 自己决定)。
...
`DeactivateSkill` / `ReplaceActiveSkills`)会以 `FacadeError::Config` 显式拒绝,因为
facade 尚无 skill registry 与 skill-to-prompt/tool 展开层——请求内容问题(含不支持的
请求 family 与非法 `SetModel` payload)一律报 `Config`,时机问题(active/parked turn)
才报 `InvalidState`,两者可区分。facade 选择的时机语义比底层 machine
...
构造全部已支持请求只需要 facade 重导出的类型:`ModelRef`、`ToolSetId`、`ToolSetRef`、
`ToolSetPatch`、`ToolDecl`(data-only 工具声明,区别于可执行的 typed `Tool`)与
`LoopPolicy`/`ToolFailurePolicy` 都在 `agent_lib::facade` 下可用,facade-only 消费者不需要 import
`agent::` 或 `model::` 内部模块(M2-4)。

已准入的 reconfig 仍只在下一 turn 边界应用。`SetModel` 与 `SetSystemPromptOverlay` 会在下一次
LLM request 渲染前落入 `AgentState`;`ReplaceToolSet` / `PatchToolSet` 会经 facade 专用
`ToolRegistryResolver` 校验目标声明的名字集合必须来自该 agent 已注册的 facade tool surface
(typed tools、custom registry、delegation tools 等)。不匹配的声明会在 `Agent::reconfigure` 准入时以
`FacadeError::Agent(AgentError::Tool(..))` 失败,不会排队成一次之后才静默失配的 reconfig。

tool-set reconfig 只管**非委派**工具面(B1 加固):委派工具声明(model-routed 模式下每个
delegate 一个 `ask_<name>`,或 SingleTool 模式的统一工具名)永远由 facade 从**当前注册的
delegates** 重新合成,不取自调用方。`ReplaceToolSet` 入队前,facade 把调用方集合与重合成的委派
声明合并后再交给 state 层,因此逐字替换的语义只作用于非委派部分——调用方替换插件工具集时不可能
意外把仍注册的 delegate 从 tool surface 上弄丢。委派声明是派生状态而非调用方输入:替换集自带与
委派工具同名的声明、或 patch 试图 remove / add-or-replace 一个委派工具名,都在准入时以
`FacadeError::Config` 显式拒绝(要下线委派工具,删除 delegate 注册或在 restore 时
`prune_unregistered_delegates`,而不是编辑它的工具声明)。delegate 集合本身变化(注册新
subagent/external_agent、restore prune)时,声明派生走 build/restore 的既有路径,与 reconfig
合并规则天然一致。

准入校验是**按名字**的:tool-set reconfig 只检查声明名是否属于已注册 tool surface,声明的
description / input_schema 内容是调用方提供的展示元数据,会原样渲染进下一次 LLM request
的 `tools` 字段;执行永远使用注册表里该名字的闭包。因此同名但 schema 不同的声明会被接受
——这是有意设计:执行语义以注册闭包为权威,声明内容只影响模型看到什么,调用方对两者一致
性负责。

`SetLoopPolicy` 会**整体替换** facade 从 builder 派生的 step budget
(`min(max_steps, max_tool_rounds + 1)`,见 §8.4):reconfig 之后 builder 的
`max_steps` / `max_tool_rounds` 不再参与计算,loop policy 以调用方给出的值为准。

`SetModel` 的准入校验与 `AgentBuilder::build` 对初始 model 的校验对齐(M2-4):model 名非空白、
temperature 有限、provider_extras 与可推断的 provider 一致(facade 不保留 builder 的
`ProviderConfig`,因此以当前 model 的 provider_extras 所指向的 provider 为准;两侧都无 extras
时按 builder 的 client-only 逃生口语义放行,由注入 client 自行决定)。非法输入以
`FacadeError::Config` 在准入时失败,不排队。

非流式 `run` / `run_full` 与流式 `stream` 都接线同一个 active registry slot:每次 run 开始时按
`state.current_tool_set()` 过滤可执行 registry,遇到 turn-boundary tool-set reconfig 时再由
`ReconfigRegistryHandler` 换入新的过滤 registry。因此后续 LLM request 的 `tools` 字段、
`Agent::state().current_tool_set()` 与实际可执行工具保持一致;被移除的工具即便被模型错误调用也会得到明确
`UnknownTool` tool result,不会继续执行旧闭包。reconfig 结果不新增 `RunEvent` 变体,可通过返回后的
`AgentState`、下一次记录到的 LLM request tools/system/model 以及工具错误结果观察。

snapshot/restore 与 reconfigure 的分工(M2-4 测试钉住):**restore 是换 provider / client /
runtime handler(工具闭包、approval handler、interaction handler)的路径,reconfigure 是换
model / tools / system 的路径**,二者不互相替代。restore 以快照 `AgentState` 为权威——已应用的
reconfig(新 model、新 tool set)随快照保留,恢复的 agent 直接使用;重注入不同工具集只按名字
替换执行闭包,模型看到的声明仍以快照为准。排队未应用的 reconfig 也是 `AgentState` 的一部分,
会被快照捕获并在 restore 后于下一 turn 边界照常应用(见 §15.2)。restore 额外校验重注入的工具面
能执行快照可激活的全部 tool set(当前集合 + 排队 reconfig 的应用结果),不满足即 restore 显式
失败,避免恢复出每次 run 都在 registry 解析处失败的锁死 agent。
...
除声明面校验外，restore 还校验重注入工具面的**可执行覆盖**（M2-4）：快照 `AgentState` 是数据权威，
其 `current_tool_set` 与排队未应用的 reconfig 队列（队列本身是序列化状态的一部分，restore 时重新
plan）所指向的每一个 tool set，其名字集合都必须 ⊆ 重注入的 facade tool surface。因为每次 run 在
drain 开始前就按 `current_tool_set` 解析 active registry，工具面偏小会让每次 run 在排队矫正
reconfig 有机会应用之前就失败，agent 被永久锁死（`into_parts` 之外无解）；restore 选择在
`AgentRestoreBuilder::build()` 以 `FacadeError::InvalidState` 显式失败，使该状态在公开 API 上不可
表示——调用方补足重注入工具后重试即可。

排队未应用的 reconfig 的快照语义（M2-4 钉死）：`Agent::snapshot()` 在 reconfig 已排队但尚未到
turn 边界应用时调用，**队列随 `AgentState` 一起被捕获**；restore 后的 agent 保留该队列，在下一次
run 的 turn 边界照常应用，行为与从未取过快照一致（不丢弃、不提前应用）。

Delegate 合并语义与 auto_allow 警告（B4）：snapshot 不携带 delegate 的 `ApprovalPolicy`（可能含
闭包的运行时句柄），restore 时对未被重注册的 delegate 一律回落 `ApprovalPolicy::default()` =
**auto_allow**；`.subagent(..)` / `.external_agent(..)` 重注册只按名**覆盖**，从不删除。因此默认行为
下，host 在两次进程之间从配置中**删除**的 delegate 会在 restore 后静默复活：仍注册在 agent 上、
`ask_<name>` 仍广告在 tool surface、且以免审批的 auto_allow 运行。若 host 的当前配置才是 delegate
集合的权威，使用 opt-in 的 `AgentRestoreBuilder::prune_unregistered_delegates()`：开启后最终
delegate 集合 = host 重注册集合（local 与 external 都覆盖），被裁剪 delegate 的合成 `ask_<name>`
声明从恢复出的 current/initial tool set 中同步移除（不再出现在模型可见 tool surface），external
delegate 的 retained session facts 一并丢弃。裁剪不影响 snapshot 其余部分（对话历史、model、
system prompt、loop policy、reconfig 队列、协作 slice、非委派工具声明照常恢复）；排队未应用的
reconfig 若仍引用被裁剪 delegate 的工具，restore 以 `FacadeError::InvalidState` 显式失败而不静默
改写。**默认行为不变**：不加 prune 时 snapshot 的全部 delegate 照常恢复，未重注册者回落 default
...
若 host 的当前配置是 delegate 集合的权威（两次进程之间可能**删除**过 delegate），加
`.prune_unregistered_delegates()`：未被重注册的 snapshot delegate 会被裁剪而不是以免审批的
auto_allow 复活（语义与警告见 §15.2 的 B4 段）。
