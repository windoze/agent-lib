...
- **关键差异——每 turn 一个一次性进程**:与 Claude Code 的单条长驻 `stream-json` 进程不同,`codex exec`
  的 prompt 是 CLI **位置参数**(不是 stdin 帧,统一以 `--` 分隔符前置,见 §16「prompt argv 暴露面」),进程
  在一个 turn 落定后即退出;续跑是全新的
...
> - **每 turn 一个一次性进程**(同 Codex):`opencode run` 的 prompt 是 CLI **位置参数**(以 `--` 分隔符前置,
>   见 §16「prompt argv 暴露面」),进程在一个 turn 落定
...
### prompt argv 暴露面（M2-4 / M-EXT-4）

Codex / OpenCode 的 prompt 以 CLI **位置参数**传递（Claude Code 走 stdin frame，无此问题），涉及两点：

- **`-` 前缀误解析（已修复）**：以 `-` 开头的用户消息（如 `--model`）曾被 clap/yargs 当 flag 解析。
  M2-4 起两个 adapter 在 prompt/message 前统一插入 `--` 分隔符。支持性确认：Codex 为 clap 原生支持
  （本机实测 `codex exec --help` / `codex exec resume --help`）；OpenCode 的 yargs 入口配置
  `parserConfiguration({ "populate--": true })`（sst/opencode `src/index.ts` 源码确认），`--` 之后的词
  进入 `message` 位置数组。
- **`ps` 可见性（已知残留暴露面，维持 argv）**：argv 对同主机其他用户的 `ps` 可见，prompt 中的敏感
  代码/文本存在本机暴露面。评估过 stdin 传参（Codex `exec` 支持以 `-` 为 prompt 从 stdin 读），未采用：
