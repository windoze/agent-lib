...
- **local subagent 路径**：`FacadeSubagentSpawner::spawn`（`src/facade/delegate.rs:1465`）为子
  agent 新建**同步** `FacadeApproval`（worker 自带 `ApprovalPolicy`），接线为子作用域的
  `InteractionHandler`（`ChildAgentScope`，`delegate.rs:1374-1392`）；外层 pop 目标是**无父级
  的** `EmptyScope`（`delegate.rs:1738-1739`）。`Approval::ask` 是同步闭包
  （`facade/approval.rs:210`），headless 下 ask 即 deny。
- **external ACP 路径**：ACP adapter 真实地把 `session/request_permission` 桥接为
  `RuntimeDecisionPoint::PausedForInteraction`（携带 `Interaction::permission`，
  `agent/external/acp/adapter.rs:446-465, 500-511`），machine 具现为 `NeedInteraction`
  （`agent/external/machine.rs:765-806`）；但 facade 委派驱动（`drive_external`，
  `facade/external.rs:1681`）给外层接的是 `EmptyExternalScope`（`external.rs:1727-1728`）——
  代码注释自述 "The richer external approval wiring lands in M4-3"（`external.rs:1544-1550`），
  **未落地**。结果：ACP 子 agent 发起权限请求 → `UnhandledRequirement` → **整个委派失败**。
- **父级注入的 handler 对子 agent 完全不可见**：`AgentBuilder::interaction_handler(..)`
  （`facade/agent.rs:1290`）只装进 supervisor 作用域（`agent.rs:504-508`，流式
  `stream.rs:441`）。
- **无归因**：`Interaction { step_id, kind }`（`agent/interaction.rs:46-52`）没有 delegate
  归属字段；`PermissionRequest.actor: AgentId`（`agent/permission.rs:98`）存在且 ACP adapter
  会绑定（`acp/adapter.rs:122-124`），但该请求到不了 facade handler。
...
5. external 的 `ExternalPermissionMode::Prompt` 在路由落地后端到端可用：ACP 子 agent
   `session/request_permission` → 父级 handler 应答 → outcome 回灌 ACP transport → 委派继续。

### 关联：external-start approval（可降级）

`FacadeApproval::resolve_external_start`（`approval.rs:573`，调用点 `delegate.rs:1825`）是
**sync-only**：`ask_external_agents()` 在无同步 ask handler 时 headless deny。mag 可以用
`ApprovalPolicy::ask_tool("ask_<name>")` 把「是否启动委派」经父级异步 handler 审批（工具门），
因此本项**可降级为文档承诺**（明确 external-start 保持 sync、推荐工具门模式）；若评估后认为
异步化成本低，可一并做。
...
- **C5**（mag A9）external-start approval 走异步 handler（见 A1 关联节，可降级为文档承诺）。
