...
注入 handler 已落地（✅ 已修复，M1-2）。external ACP/CLI 委派路径仍按 M1-3 处理，external-start
approval 去留仍按 M1-4 评估。
...
- **父级注入 handler 对 local 子 agent 可见**（✅ 已修复，M1-2）：`AgentBuilder::interaction_handler(..)`
  的注入 handler 会传入 local 委派驱动；external 委派路径仍未接入（M1-3）。
