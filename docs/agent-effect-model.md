...
- **两个观测点**:`drain` 在每批 fulfil 之前和 fulfil 返回之后(resume 之前)各检查
  一次取消。后者把"飞行中被取消"的批次在回灌前截停:已兑现的结果被有意丢弃、按
  never-resume 留痕,turn 不再向前多推进一批,取消延迟以批次 settle 为界。
- **取消延迟有界**:参考实现的 LLM handler 把取消令牌接入等待(与 LLM 调用 `select!`
  竞争),取消信号到达即放弃在途请求,延迟以信号投递为界而非模型响应时长;tool 执行
  有意不中途打断(半途副作用比有界等待更糟)。
