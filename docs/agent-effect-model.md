...
已知挂起态要收尾"会记得它。** 这正是"cancel 后 conversation 仍可 feed"这条硬性验收
的实现基础:never-resume 是**受控丢弃 + 闭合**,不是撒手不管。

**取消延迟(M3-3)。** driver 不等一个阻塞中的批跑完才响应 cancel:批级等待本身可被
抢占。cancel 命中在途批时,driver 先继续 poll 该批一个有界宽限
(`CANCEL_UNWIND_GRACE`,2s)——合作的 handler(LLM、external session 读循环、自行
select cancel 的工具)会在这个窗口内收尾,external 委派的会话清扫得以执行;宽限到期仍
阻塞的 fulfill future 被**drop(detach)**,批内全部 requirement 按 never-resume 留痕并
abandon,turn 以 cancelled 收尾。因此被 detach 的 handler 不能依赖"跑到返回":长工具必须
自行 select 取消令牌(facade 工具经 `ToolContext.cancel`,agent 层 handler 经
`RunContext::cancellation`),阻塞中的 `ask_user` 类交互 handler 同理。
...
取消侧的 settle 完整性、观测点与 `TurnDone` 契约(M4-5 / M-ERR-2 收口):

- **批内全部留痕**:取消命中时,批次内**每一个** outstanding requirement 都以
  `NeverResumed` 记录并逐一 `Abandon`——首个 Abandon 经机器的 never-resume 路径闭合
  整轮,同批后续 Abandon 被软拒绝为 no-op(M4-4),但 trace 留痕一个不落。只记录批次
  第一个即违反"每个 requirement 恰好以一种方式 settle 并记录"。
- **三个观测点**:`drain` 在每批 fulfil 之前、批次等待**期间**(M3-3,见 §6.3「取消
  延迟」)和 fulfil 返回之后(resume 之前)各检查一次取消。批等待期间的观测点把
  阻塞中的批本身变成可抢占:cancel 命中后先给合作 handler 一个有界 unwind 宽限
  (`CANCEL_UNWIND_GRACE`,2s)跑清理尾,宽限到期仍阻塞的 fulfill future 被
  drop(detach)。批后观测点把"飞行中被取消"的批次在回灌前截停:已兑现的结果被有意
  丢弃、按 never-resume 留痕,turn 不再向前多推进一批。
- **取消延迟有界**:参考实现的 LLM handler 把取消令牌接入等待(与 LLM 调用 `select!`
  竞争),取消信号到达即放弃在途请求,延迟以信号投递为界而非模型响应时长;tool /
  interaction 批的等待同样可被抢占,最坏延迟收紧为 unwind 宽限(2s)而非批内最慢
  handler 的返回时间——被 detach 的 handler 不得依赖"跑到返回",长工具必须自行
  select 取消令牌(§6.3)。
- **`TurnDone` 契约**:自然结束 → terminal `Done|Error` cursor 且 `cancelled()==false`;
  取消 → 机器停在 never-resume 后的 rest 状态(参考机器为 `Idle`)且 `cancelled()==true`。
  区分取消与自然结束必须看 `cancelled()`,不能只靠 cursor 是否 terminal。
