//! Local HTTP coverage for the public Anthropic streaming entry point.
...
fn local_adapter(base_url: String) -> AnthropicAdapter {
    AnthropicAdapter::new(EndpointConfig {
        base_url,
    ...
    ChatRequest {
        ...
        messages: vec![Message {
            ...
            content: vec![ContentBlock::Text {
                text: "Count from 1 to 5.".to_owned(),
                extra: Map::new(),
            }],
        }],
        tools: Vec::new(),
...
async fn boxed_client_sends_sse_request_and_yields_foldable_events() {
    ...
    let events = collect_with_timeout(client.as_ref(), request(true))
        .await
        .expect("streaming Anthropic chat should succeed");
    ...
    assert_eq!(response.usage.input, 22);
    assert_eq!(response.usage.output, 13);
}
