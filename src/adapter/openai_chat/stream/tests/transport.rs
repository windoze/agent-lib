//! Local HTTP coverage for the public Chat/Completions streaming entry point.
...
async fn serve_once(
    ...
    let task = tokio::spawn(async move {
        ...
        assert!(
            ...
            "streaming request should target chat/completions: {request}"
        );
...
fn local_adapter(base_url: String) -> OpenAiChatAdapter {
...
/// Bounds complete event consumption so a transport regression cannot hang the
/// test.
...
#[tokio::test]
async fn streaming_chat_classifies_rate_limit_with_retry_after() {
...
#[tokio::test]
async fn streaming_chat_classifies_generic_http_error_body() {
    let body = r#"{"error":{"code":"server_error","message":"boom"}}"#;
    let (base_url, server) =
        serve_once("500 Internal Server Error", "application/json", &[], body).await;
    let adapter = local_adapter(base_url);

    let error = match adapter.chat_stream(request(true)).await {
        Err(error) => error,
        Ok(_) => panic!("server error should fail before returning a body stream"),
    };
    server.await.expect("local error server should finish");
...
#[tokio::test]
async fn truncated_body_surfaces_incomplete_done_error() {
    let truncated = "data: {\"choices\":[{\"index\":0,\"delta\":{\"content\":\"hi\"},\"finish_reason\":\"stop\"}]}\n\n";
