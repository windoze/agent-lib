...
async fn serve_once(
    ...
    let task = tokio::spawn(async move {
        ...
        assert!(
            ...
            "adapter should call the Responses path: {request}"
        );
        socket
...
async fn chat_sends_and_parses_non_streaming_response() {
    ...
    let adapter = OpenAiRespAdapter::new(local_endpoint(base_url));

    let response = chat_with_timeout(&adapter, minimal_request())
        .await
        .expect("complete OpenAI Responses chat should succeed");
...
async fn chat_rejects_invalid_success_body() {
    ...
    assert!(error.to_string().contains("OpenAI Responses response"));
}

#[tokio::test]
async fn chat_rejects_streaming_request_before_transport() {
    let adapter = OpenAiRespAdapter::new(local_endpoint("http://127.0.0.1:1".to_owned()));
