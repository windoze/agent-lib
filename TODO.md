...
- 对照 `docs/conversation-core.md` §10 与 `PLAN.md` 的 Serde / 持久化边界完成人工审查：
  `ConversationSnapshot` 只记录 committed consistency point 的 id/config、raw closed Turn facts、
  lineage/head/version、fork origin、projection spans/artifact provenance；`Conversation::snapshot`
  在存在 pending 时分类拒绝，且 persisted shape 排除 `PendingTurn`/`PendingMessage`、
  Client `Accumulator`、`ToolCallIndex`、锁、client/registry handle 与 runtime strategy/trigger
  object。外部 id/time 仍来自调用方注入的 Turn/message/meta facts，持久化路径不调用随机源或
  时钟。
- 审查确认 restore 没有绕过 public commit API 的第二条 live history 入口：
  `Conversation::restore` 先执行 schema version gate，再把每个 raw `TurnData` 送回同一个
  I1--I4 validator，随后校验 parent graph、lineage/head/fork ceiling、fork origin 与
  restored projection/artifact provenance，最后从 closed facts 重建 `History` 和派生
  `ToolCallIndex`；未知 schema、重复/损坏 Turn facts、缺失/循环 parent、坏 lineage/head、
  坏 fork origin、projection mismatch 与 derived snapshot fields 均有分类拒绝测试。
- 审查确认 `ConversationRows` 是 DB-neutral data-only 映射而非 live restore 入口：
  rows reassembly 只做 row-level PK/FK/owner/dense sequence/projection data-shape 检查并返回
  `ConversationSnapshot`，调用方仍必须进入 `Conversation::restore`；`insert_set_against` 对同
  PK 不同 immutable fact 返回 `RowMappingError::InsertConflict`，不会表达 UPDATE。public
  rustdoc 明确 rows 不绑定数据库 driver，message/turn rows 为 insert-only，annotation/评分应
  另表引用 `MessageId`。
- 人工追踪 M5-4 的 fork+compaction e2e fixture：同一多 Turn、serial/parallel tools、
  tiered/consolidated compaction、revert/redo、fork 后父子独立追加的会话，分别经 JSON
  snapshot 与 DB-neutral rows（含 rows serde 与打乱读取顺序）恢复；共享祖先 Turn/message id
  未复制或重分配，fork child 不继承父 suffix/projection，恢复后 system、raw/current lineage
  facts、head/boundaries、origin、projection/provenance、usage、rebuilt index 与
  `effective_view` 均与原对象一致，head 落入 compacted cover 时仍不泄漏未来摘要。
- 阶段级计划、依赖和完成标准未变化，故未修改 `PLAN.md`；本 review 未发现需要新增前置任务的
  阻塞缺口。
- 验证通过：`cargo fmt --all`；`cargo clippy --all-targets -- -D warnings`；
  `cargo test conversation::persistence -- --nocapture`（18 passed，覆盖 corruption、pending
  snapshot gate、rows round-trip、fork/compaction/effective_view e2e）；1800 秒硬上限内
  `perl -e 'alarm 1800; exec @ARGV' cargo test --all --all-targets`（287 个库测试与 3 个离线
  集成测试 passed、7 ignored、0 failed，所有 example targets passed）；
  `RUSTDOCFLAGS="-D warnings" cargo doc --no-deps`；`git diff --check`。
...
### M6-1 [DONE] Conversation 状态机组合验收
...
- 新增外部集成测试 `tests/conversation_state_machine.rs`、fixture/helper 模块
  `tests/conversation_state_machine/support.rs` 与断言模块
  `tests/conversation_state_machine/assertions.rs`，只通过 crate 公共 API 驱动 Conversation；测试内
  使用 deterministic UUID、显式 normalized `Response`/`StreamEvent`、`ToolResponse`、Boundary、
  fork、compaction plan、snapshot/rows restore 等公开入口，不使用 crate-private draft、validator
  或测试专用构造器。
- 新增统一状态机断言：每个关键 transition 后检查 current closed Turn 的 user 起始、无 system、
  assistant 终止且 final assistant 无 tool-use、显式 pairing 与 tool-use/tool-result content 双向
  对齐、current parent chain/head clipping、`ToolCallIndex::rebuild` 等价、projection span
  连续性和 artifact 引用；失败操作用 observable runtime snapshot 比较，确保 history、pending、
  projection、index 和 version 原子不变。pending 阶段额外断言 Boundary 与 range 消费按规范分类
  拒绝，而 committed consistency point 可重新验证。
- 组合场景覆盖 M6-1 指定矩阵：streaming parallel tool-use 冻结后注册 mapping、部分 tool result
  完成、`CancelDisposition::ResumeTurn` 合成 `ToolStatus::Cancelled` result 并继续 final assistant
  commit；compacted history 内 revert 到 summary cover 前缀后 fork child，验证无未来摘要泄漏；
  parent/child 后续分别 local commit、raw/span compaction、JSON snapshot restore 与 DB-neutral rows
  restore，确认 projection/provenance、effective view、tool index 与 raw facts 保持一致且父子 suffix
  互不泄漏；stale Boundary 与损坏 schema snapshot 均明确失败且原 Conversation 随后可 fresh redo
  并完成新的纯文本 Turn。
- 验证通过：`cargo fmt --all`；`cargo clippy --all-targets -- -D warnings`；
  `cargo test --test conversation_state_machine -- --nocapture`（3 passed）；
  1800 秒硬上限内 `cargo test --all --all-targets`（287 个库测试、3 个
  `capability_escape_hatches` 集成测试、3 个新增 `conversation_state_machine` 集成测试通过，7 个
  真实 endpoint 测试 ignored，所有 example test target 编译通过）；
  `RUSTDOCFLAGS="-D warnings" cargo doc --no-deps`；`git diff --check`。

### M6-2 [DONE] 两家 Client request mapper 兼容性验收
...
- 新增外部集成测试 `tests/conversation_adapter_compat.rs`，只通过 public Conversation API 构造
  canonical history：包含独立 system prompt、已压缩的多 Turn artifact、可见 raw suffix 中的
  assistant reasoning、parallel tool calls、`ToolStatus::Denied` tool result，以及 cancel resume
  合成并最终提交的 `ToolStatus::Cancelled` result。测试从同一个
  `Conversation::effective_view()` 生成 `ChatRequest`，没有读取 pending partial、crate-private
  draft 或 adapter 专属 Conversation 分支。
- 同一 `EffectiveView` 分别交给 `AnthropicAdapter::build_request` 与
  `OpenAiRespAdapter::build_request` 的本地 dummy endpoint；两条路径均只构造 buffered
  `reqwest::Request`，不发网络。统一断言检查 normalized role/content/tool-use/tool-result pairing、
  system 单列、compaction artifact rendering、reasoning 保留和 Denied/Cancelled 事实仍留在
  Conversation view；provider 专属 helper 仅检查 Anthropic `tool_result.is_error=true` 与
  OpenAI `function_call_output.status=incomplete` 等 wire 差异。
- 新增 public API 负向回归：冻结了 tool-use 但未注册/闭合 tool result 的 pending turn 无法
  `commit_pending`，失败保持 pending 与 committed state 原子不变，且 committed
  `effective_view` 为空，不会把 dangling call 交给 adapter 作为最后防线；discard 后仍可继续提交
  follow-up Turn，证明会话未 poisoned。
- 本任务未修改生产代码、`PLAN.md` 或 Client API；仅新增跨 adapter 验收测试并更新任务记录。
- 验证通过：`cargo fmt --all`；`cargo test --test conversation_adapter_compat -- --nocapture`
  （2 passed、无 warning）；`cargo clippy --all-targets -- -D warnings`；1800 秒硬上限内
  `perl -e 'alarm 1800; exec @ARGV' cargo test --all --all-targets`（287 个库测试、3 个
  `capability_escape_hatches` 集成测试、2 个新增 `conversation_adapter_compat` 集成测试、3 个
  `conversation_state_machine` 集成测试通过，7 个真实 endpoint 测试 ignored，所有 example test
  target 编译通过）；`RUSTDOCFLAGS="-D warnings" cargo doc --no-deps`；`git diff --check`。

### M6-3 [DONE] Conversation 示例、README 与 crate 文档
...
- 新增离线示例 `examples/conversation_core.rs`，只使用公开 Conversation API、deterministic
  UUID 和本地 normalized `Response`/`ToolResponse` fixture；示例不访问真实 endpoint、不创建
  Agent loop/tool registry，也不依赖 runtime strategy registry。运行时依次断言
  user→assistant tool-use→tool-result→final assistant→commit、`CancelDisposition::ResumeTurn`
  合成 `ToolStatus::Cancelled` 后继续 feed、合法 `Boundary`/`fork_at`、raw compaction 后
  `effective_view` 渲染 summary artifact，以及 `Conversation::snapshot` JSON round-trip 后
  `Conversation::restore` 的 effective view/raw facts 一致。
- 更新根 `README.md` 的当前实现状态、设计边界和可运行示例入口：保留 Client endpoint 配置和
  已归档 Client 层链接，新增 `cargo run --example conversation_core` 的无环境变量用法，并明确
  summarizer、Agent loop、Tool registry、自动预算调度、数据库 driver 与多 agent 编排仍不属于
  Conversation Core。
- 更新 `src/lib.rs` crate docs，新增可运行的 Conversation Core 最小示例，展示外部 id 注入、
  pending assistant freeze、`commit_pending` 和 `effective_view`；为关键公共边界补充 rustdoc
  示例：`Boundary` serde 后仍需 `validate_boundary`、`fork_at` 使用 child owner/version、
  `CancelDisposition::ResumeTurn` 的 synthetic cancelled result identity bundle，以及 pending
  存在时 `Conversation::snapshot` 分类拒绝。
- 使用 `rg` 审查陈旧“Client 层当前计划/任务”和 Conversation 状态引用；仅修正 README 中与新增
  离线示例冲突或状态过期的描述，保留仍正确指向根 `PLAN.md`/`TODO.md` 的当前阶段入口和归档
  Client 层链接。阶段级计划、依赖和完成标准未变化，因此未修改 `PLAN.md`。
- 验证通过：`cargo fmt --all`；`cargo clippy --all-targets -- -D warnings`；
  `cargo run --example conversation_core`（输出 `parent_turns=2`、`child_turns=2`、
  `snapshot_bytes=8854`）；`cargo test --doc`（6 个正向 doctest 与 10 个 compile-fail doctest
  passed）；1800 秒硬上限内 `perl -e 'alarm 1800; exec @ARGV' cargo test --all --all-targets`
  （287 个库测试、3 个 capability 集成测试、2 个 conversation adapter 兼容测试、3 个
