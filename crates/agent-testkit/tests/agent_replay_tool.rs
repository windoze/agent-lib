//! Recorded replay suite (`agent_replay_tool`, milestone 6, M6-4).
//!
//! A full `user -> LLM tool_use -> tool result -> LLM final text` round-trip run
//! entirely off a committed cassette
//! ([`agent_tool_error_roundtrip.json`](../tests/cassettes)) with **no network,
//! credentials, or live tool backend**.
//!
//! This suite deliberately records the *tool-error* path — the tool returns a
//! model-visible `Error` result that the model recovers from — so it covers a
//! different observable flow than the happy-path weather round-trip already
//! proven by [`cassette_replay`](../tests/cassette_replay.rs) (M3-4): here the
//! committed conversation carries a `ToolStatus::Error` result rather than an
//! `Ok` one, and the offline replay reproduces that error hand-off without a
//! live backend.
//!
//! The committed fixture is *synthetic* recorded data written in the exact shape
//! [`CassetteRecorder`] produces: [`regenerate_tool_cassette`] records the same
//! scenario through the recorder and, only when
...
//! actually issues keeps the entry fingerprints in lock-step with what the
...
//!
//! Run in isolation with `cargo test --test agent_replay_tool`.
...
/// The tool's recorded, model-visible *error* reply.
const TOOL_ERROR: &str = "weather backend unavailable";
/// The assistant's final answer after recovering from the tool error.
const FINAL_ANSWER: &str = "Sorry, I could not reach the weather service.";

/// Absolute path to the committed cassette fixture.
fn cassette_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("tests/cassettes/agent_tool_error_roundtrip.json")
}

/// The scripted LLM handler: first ask for `get_weather`, then answer once the
/// (failed) tool result returns. Usages are non-zero so the recorded result
...
/// The scripted tool handler that fails the single `get_weather` call with a
/// model-visible `Error` result.
fn scripted_tool() -> ScriptedToolHandler {
    ScriptedToolHandler::from_steps([ToolStep::error(CALL_ID, TOOL_ERROR)])
}

/// Records the tool-error round-trip through [`CassetteRecorder`] and rewrites
/// the committed fixture — but only when `AGENT_TESTKIT_UPDATE_CASSETTES=1`.
///
...
#[tokio::test]
async fn regenerate_tool_cassette() {
    let recorder = CassetteRecorder::update(cassette_path()).with_metadata(
        CassetteMetadata::new("agent_tool_error_roundtrip").with_description(
            "user -> LLM tool_use -> get_weather error result -> LLM recovery text; no network or credentials",
        ),
    );
    ...
    .await
    .expect("the recorded tool-error turn drains to completion");
...
/// [`CassetteToolHandler`] serve every requirement, so there is no network,
/// credentials, or real tool. The assertions cover the three things the task
/// calls out — the committed conversation, the handler call logs, and the final
/// cursor — and confirm the tool result was recorded and replayed as `Error`.
#[tokio::test]
async fn offline_replay_runs_a_tool_error_turn() {
    ...
    let player = CassettePlayer::new(cassette, "tests/cassettes/agent_tool_error_roundtrip.json");
    ...
    .await
    .expect("the offline tool-error turn drains to completion");
    ...
    // and no interaction/reconfig traffic (those handlers do not exist).
    assert_calls(&llm_log).count(2).all_completed();
    assert_calls(&tool_log).count(1).all_completed();

    // Committed conversation: one closed turn of user, assistant tool-use, the
    // failed tool result, and the assistant's recovery answer, with the single
    // tool call paired and its result recorded as `Error`.
    assert_conversation(machine.state().conversation())
        ...
        .tool_result_status(CALL_ID, ToolStatus::Error)
        .last_assistant_text(FINAL_ANSWER);

    // The assertion module intentionally stays out of block-level detail, so the
    // last low-level facts — the exact message count and the tool-use *name* —
    // are still checked directly against the committed conversation.
    let messages = machine.state().conversation().turns()[0].messages();
    assert_eq!(
        messages.len(),
        4,
        "user, tool-use, failed tool result, recovery answer"
    );
